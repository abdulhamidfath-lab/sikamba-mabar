export default async function handler(req, res) {
  ...
  if (!id) {
    res.writeHead(302, { Location: `${BASE_URL}/pages/data-ikm.html` });
    res.end();
    return;
  }
  ...
  try {
    const resp = await fetch(
      `${SUPABASE_URL}/rest/v1/ikm?id=eq.${id}&select=*`,
    ...
    const judul = `${ikm.nama_usaha} — SIKAMBA MABAR`;
    const deskripsi = `${ikm.kategori} — ${[ikm.desa, ikm.kecamatan].filter(Boolean).join(', ') || 'Manggarai Barat'}`;
    const gambar = ikm.foto_url || `${BASE_URL}/images/logo-mabar.png`;
    const urlAsli = `${BASE_URL}/pages/detail-ikm.html?id=${id}`;
